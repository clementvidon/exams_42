...
char	*ft_strdup(char const *s1)
{
	...
	if (!s1) 			// XXX null check
		return (NULL);
	s2 = malloc (sizeof (*s2) * (ft_strlen (s1) + 1)); // XXX sizeof char not char *
	if (!s2)
		return (NULL);
	p2 = s2;
	while (*s1)
...
char	*ft_strjoin_free_s1(char *s1, char const *s2)
{
	...
	if (s1 && !s2)
		return (ft_strdup (s1));
	if (!s1 && !s2)
		return (ft_strdup ("")); // dont return NULL!
	s3 = malloc (sizeof (*s3) * (ft_strlen (s1) + ft_strlen (s2) + 1));
	if (!s3)
		return (free (s1), NULL); // XXX free s1 !!
	p3 = s3;
	p1 = s1;
	while (*p1)
		*p3++ = *p1++;
...
char	*ft_substr(char const *str, unsigned int start, size_t size)
{
	...
	if (!sub)
		return (NULL);
	sub[size] = 0;
	while (size--) // XXX use subscript operator
...
int	main(void)
{
	...
	/* dprintf(2, "stjoin> %s == \n", join); */
	/* free (join); */
	/* join = ft_strjoin_free_s1(ft_strdup("s"), "u"); */
